...
export async function NTHU_login(account, password) {
    // 取得驗證碼ID與圖片 (fnstr)
    const fnstr = await captcha()
        .catch((err) => {
    ...
    const getPw2 = (async () => {
        ...
        if (num > 999999)
            throw new Error('超出六位數範圍'); // TODO: 更嚴謹的驗證
        return num;
    })().catch(async () => {
